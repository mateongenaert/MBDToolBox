...
TSScounts<-matrix(nrow=189845,ncol=61)
TSScounts<-as.data.frame(TSScounts)
...
TSScounts[,3]<-DNA277.Overlap
rm(DNA277);gc()

DNA511<-scanBam("511_meth_nodups.bam", param = params)[[1]]
...
TSScounts[,4]<-DNA511.Overlap
rm(DNA511);gc()

DNA520<-scanBam("520_meth_nodups.bam", param = params)[[1]]
...
TSScounts[,5]<-DNA520.Overlap
rm(DNA520);gc()

DNA610<-scanBam("610_meth_nodups.bam", param = params)[[1]]
...
TSScounts[,6]<-DNA610.Overlap
rm(DNA610);gc()

DNA750<-scanBam("750_meth_nodups.bam", param = params)[[1]]
...
TSScounts[,7]<-DNA750.Overlap
rm(DNA750);gc()

DNA820<-scanBam("820_meth_nodups.bam", param = params)[[1]]
...
TSScounts[,8]<-DNA820.Overlap
rm(DNA820);gc()

DNA821<-scanBam("821_meth_nodups.bam", param = params)[[1]]
...
TSScounts[,9]<-DNA821.Overlap
rm(DNA821);gc()

DNA822<-scanBam("822_meth_nodups.bam", param = params)[[1]]
...
TSScounts[,10]<-DNA822.Overlap
rm(DNA822);gc()

DNA823<-scanBam("823_meth_nodups.bam", param = params)[[1]]
...
TSScounts[,11]<-DNA823.Overlap
rm(DNA823);gc()

DNA912<-scanBam("912_meth_nodups.bam", param = params)[[1]]
...
TSScounts[,12]<-DNA912.Overlap
rm(DNA912);gc()

DNA928<-scanBam("928_meth_nodups.bam", param = params)[[1]]
...
TSScounts[,13]<-DNA928.Overlap
rm(DNA928);gc()

DNA1013<-scanBam("1013_meth_nodups.bam", param = params)[[1]]
...
TSScounts[,14]<-DNA1013.Overlap
rm(DNA1013);gc()

DNA1028<-scanBam("1028_meth_nodups.bam", param = params)[[1]]
...
TSScounts[,15]<-DNA1028.Overlap
rm(DNA1028);gc()

DNA1030<-scanBam("1030_meth_nodups.bam", param = params)[[1]]
...
TSScounts[,16]<-DNA1030.Overlap
rm(DNA1030);gc()

DNA1038<-scanBam("1038_meth_nodups.bam", param = params)[[1]]
...
TSScounts[,17]<-DNA1038.Overlap
rm(DNA1038);gc()

DNA1039<-scanBam("1039_meth_nodups.bam", param = params)[[1]]
...
TSScounts[,18]<-DNA1039.Overlap
rm(DNA1039);gc()

DNA1191<-scanBam("1191_meth_nodups.bam", param = params)[[1]]
...
TSScounts[,19]<-DNA1191.Overlap
rm(DNA1191);gc()

DNA1381<-scanBam("1381_meth_nodups.bam", param = params)[[1]]
...
TSScounts[,20]<-DNA1381.Overlap
rm(DNA1381);gc()

DNA1382<-scanBam("1382_meth_nodups.bam", param = params)[[1]]
...
TSScounts[,21]<-DNA1382.Overlap
rm(DNA1382);gc()

DNA1383<-scanBam("1383_meth_nodups.bam", param = params)[[1]]
...
TSScounts[,22]<-DNA1383.Overlap
rm(DNA1383);gc()

DNA1384<-scanBam("1384_meth_nodups.bam", param = params)[[1]]
...
TSScounts[,23]<-DNA1384.Overlap
rm(DNA1384);gc()

DNA1392<-scanBam("1392_meth_nodups.bam", param = params)[[1]]
...
TSScounts[,24]<-DNA1392.Overlap
rm(DNA1392);gc()

DNA1430<-scanBam("1430_meth_nodups.bam", param = params)[[1]]
...
TSScounts[,25]<-DNA1430.Overlap
rm(DNA1430);gc()

DNA1469<-scanBam("1469_meth_nodups.bam", param = params)[[1]]
...
TSScounts[,26]<-DNA1469.Overlap
rm(DNA1469);gc()

DNA1476<-scanBam("1476_meth_nodups.bam", param = params)[[1]]
...
TSScounts[,27]<-DNA1476.Overlap
rm(DNA1476);gc()

DNA1483<-scanBam("1483_meth_nodups.bam", param = params)[[1]]
...
TSScounts[,28]<-DNA1483.Overlap
rm(DNA1483);gc()

DNA1484<-scanBam("1484_meth_nodups.bam", param = params)[[1]]
...
TSScounts[,29]<-DNA1484.Overlap
rm(DNA1484);gc()

DNA1486<-scanBam("1486_meth_nodups.bam", param = params)[[1]]
...
TSScounts[,30]<-DNA1486.Overlap
rm(DNA1486);gc()

DNA1488<-scanBam("1488_meth_nodups.bam", param = params)[[1]]
...
TSScounts[,31]<-DNA1488.Overlap
rm(DNA1488);gc()

DNA1494<-scanBam("1494_meth_nodups.bam", param = params)[[1]]
...
TSScounts[,32]<-DNA1494.Overlap
rm(DNA1494);gc()

DNA1501<-scanBam("1501_meth_nodups.bam", param = params)[[1]]
...
TSScounts[,33]<-DNA1501.Overlap
rm(DNA1501);gc()

DNA1507<-scanBam("1507_meth_nodups.bam", param = params)[[1]]
...
TSScounts[,34]<-DNA1507.Overlap
rm(DNA1507);gc()

DNA1509<-scanBam("1509_meth_nodups.bam", param = params)[[1]]
...
TSScounts[,35]<-DNA1509.Overlap
rm(DNA1509);gc()

DNA1515<-scanBam("1515_meth_nodups.bam", param = params)[[1]]
...
TSScounts[,36]<-DNA1515.Overlap
rm(DNA1515);gc()

DNA1530<-scanBam("1530_meth_nodups.bam", param = params)[[1]]
...
TSScounts[,37]<-DNA1530.Overlap
rm(DNA1530);gc()

DNA1537<-scanBam("1537_meth_nodups.bam", param = params)[[1]]
...
TSScounts[,38]<-DNA1537.Overlap
rm(DNA1537);gc()

DNA1613<-scanBam("1613_meth_nodups.bam", param = params)[[1]]
...
TSScounts[,39]<-DNA1613.Overlap
rm(DNA1613);gc()

DNA1615<-scanBam("1615_meth_nodups.bam", param = params)[[1]]
...
TSScounts[,40]<-DNA1615.Overlap
rm(DNA1615);gc()

DNA1646<-scanBam("1646_meth_nodups.bam", param = params)[[1]]
...
TSScounts[,41]<-DNA1646.Overlap
rm(DNA1646);gc()

DNA1647<-scanBam("1647_meth_nodups.bam", param = params)[[1]]
...
TSScounts[,42]<-DNA1647.Overlap
rm(DNA1647);gc()

DNA1649<-scanBam("1649_meth_nodups.bam", param = params)[[1]]
...
TSScounts[,43]<-DNA1649.Overlap
rm(DNA1649);gc()

DNA1713<-scanBam("1713_meth_nodups.bam", param = params)[[1]]
...
TSScounts[,44]<-DNA1713.Overlap
rm(DNA1713);gc()

DNA1750<-scanBam("1750_meth_nodups.bam", param = params)[[1]]
...
TSScounts[,45]<-DNA1750.Overlap
rm(DNA1750);gc()

DNA1780<-scanBam("1780_meth_nodups.bam", param = params)[[1]]
...
TSScounts[,46]<-DNA1780.Overlap
rm(DNA1780);gc()

DNA1782<-scanBam("1782_meth_nodups.bam", param = params)[[1]]
...
TSScounts[,47]<-DNA1782.Overlap
rm(DNA1782);gc()

DNA1783<-scanBam("1783_meth_nodups.bam", param = params)[[1]]
...
TSScounts[,48]<-DNA1783.Overlap
rm(DNA1783);gc()

DNA1784<-scanBam("1784_meth_nodups.bam", param = params)[[1]]
...
TSScounts[,49]<-DNA1784.Overlap
rm(DNA1784);gc()

DNA1786<-scanBam("1786_meth_nodups.bam", param = params)[[1]]
...
TSScounts[,50]<-DNA1786.Overlap
rm(DNA1786);gc()

DNA1789<-scanBam("1789_meth_nodups.bam", param = params)[[1]]
...
TSScounts[,51]<-DNA1789.Overlap
rm(DNA1789);gc()

DNA1790<-scanBam("1790_meth_nodups.bam", param = params)[[1]]
...
TSScounts[,52]<-DNA1790.Overlap
rm(DNA1790);gc()

DNA1791<-scanBam("1791_meth_nodups.bam", param = params)[[1]]
...
TSScounts[,53]<-DNA1791.Overlap
rm(DNA1791);gc()

DNA1793<-scanBam("1793_meth_nodups.bam", param = params)[[1]]
...
TSScounts[,54]<-DNA1793.Overlap
rm(DNA1793);gc()

DNA1794<-scanBam("1794_meth_nodups.bam", param = params)[[1]]
...
TSScounts[,55]<-DNA1794.Overlap
rm(DNA1794);gc()

DNA1795<-scanBam("1795_meth_nodups.bam", param = params)[[1]]
...
TSScounts[,56]<-DNA1795.Overlap
rm(DNA1795);gc()

DNA1796<-scanBam("1796_meth_nodups.bam", param = params)[[1]]
...
TSScounts[,57]<-DNA1796.Overlap
rm(DNA1796);gc()

DNA1800<-scanBam("1800_meth_nodups.bam", param = params)[[1]]
...
TSScounts[,58]<-DNA1800.Overlap
rm(DNA1800);gc()

DNA1803<-scanBam("1803_meth_nodups.bam", param = params)[[1]]
...
TSScounts[,59]<-DNA1803.Overlap
rm(DNA1803);gc()

DNA1863<-scanBam("1863_meth_nodups.bam", param = params)[[1]]
...
TSScounts[,60]<-DNA1863.Overlap
rm(DNA1863);gc()

 write.table(TSScounts,file="TSScounts_MBD_2000.txt", sep="\t",row.names=TRUE, col.names=TRUE)
